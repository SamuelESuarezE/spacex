...
export const informationLaunchpads = async (launchpad) => {

    let information1 = document.querySelector("#rocketStage1");

    information1.innerHTML = /*html*/`
    <h1 style="color: var(--color--two); margin-inline: auto; margin-top: 20px">LAUNCHPAD DATA</h1>
    <hr style="color: var(--color--two); margin-block: 6px; width: 100%">
    <table style="width: 100%">
        <tr>
            <td style="color: var(--color--two)">Short Name</td>
            <td style="font-weight: bold; text-align: right">${launchpad.name || "N/A"}</td>
        </tr>
        <tr>
            <td style="color: var(--color--two)">Status</td>
            <td style="font-weight: bold; text-align: right">${launchpad.status}</td>
        </tr>
        <tr>
            <td style="color: var(--color--two)">Locality</td>
            <td style="font-weight: bold; text-align: right">${launchpad.locality || ""}</td>
        </tr>
        <tr>
            <td style="color: var(--color--two)">Region</td>
            <td style="font-weight: bold; text-align: right; font-size: 14px">${launchpad.region || ""}</td>
        </tr>
    </table>
    <hr style="color: var(--color--two); margin-block: 6px; width: 100%">
    ...
    <table style="width: 100%">
        <tr>
            ...
            <td style="font-weight: bold; text-align: right">${launchpad.latitude}</td>
        </tr>
        <tr>
            <td style="color: var(--color--two)">Longitude</td>
            <td style="font-weight: bold; text-align: right">${launchpad.longitude}</td>
        </tr>
        <tr>
            <td style="color: var(--color--two)">Launch attempts</td>
            <td style="font-weight: bold; text-align: right">${launchpad.launch_attempts}</td>
        </tr>
        <tr>
            <td style="color: var(--color--two)">Launch sucesses</td>
            <td style="font-weight: bold; text-align: right;">${launchpad.launch_successes}</td>
        </tr>
    </table>
    <hr style="color: var(--color--two); margin-block: 6px; width: 100%">
    `
    ...
    <hr style="color: var(--color--two); margin-block: 6px; width: 100%">
    <p style="text-align:center"><a href="${launchpad.wikipedia}" style="color: white;">Wikipedia</a></p>
    ...
    <hr style="color: var(--color--two); margin-block: 6px; width: 100%">
    <p style="font-size:13px">${launchpad.details || "No details about this launchpad."}</p>`

    let information5 = document.querySelector("#information__table__2")
    information5.style.padding = "20px"
    information5.innerHTML = /*html*/`
    <h1 style="color: var(--color--two); margin-top: 20px; text-align: left">ROCKETS</h1>
    <hr style="color: var(--color--two); margin-block: 6px; width: 100%">`

    for (let rocket of launchpad.rockets) {
        information5.innerHTML += /*html*/`
        <p style="font-size:13px; color: white; text-align: left">${rocket}</p>`
    }
}

export const imageLaunchpad = async(image)=>{
